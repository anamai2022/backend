    ...
    if (req.body !== null || req.body == "") {
     // console.log(req.body)
      const UploadData = new uploadModel({
        ...
        f_section: req.body.f_section,
        f_filesize: req.body.f_filesize,
        f_createdate: moment().format(SLASH_DMYHMS),
        f_type: req.body.f_type,
        f_year:req.body.f_year,
      });
      UploadData.saveAll()
        .then((result) => {
          let message = "บันทึกข้อมูลเรียบร้อยและ Upload File เรียบร้อย";
          res.status(200).send({
            message: message,
          });
        })
